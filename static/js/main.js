...
function toggleNavbar() {
    ...
    if (navbar) {
        ...
        // Save preference to localStorage
        localStorage.setItem('navbarHidden', navbar.classList.contains('navbar-hidden'));
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (sidebar && localStorage.getItem('sidebarHidden') === 'true') {
    ...
    if (navbar && localStorage.getItem('navbarHidden') === 'true') {
        ...
        // Default visible state
        if (navbarBarsIcon) navbarBarsIcon.style.display = 'none';
        if (navbarCloseIcon) navbarCloseIcon.style.display = 'inline-block';
    }

    // Add event listeners for toggle checkboxes
    const sidebarCheckboxBtn = document.getElementById('sidebarCheckbox');
    if (sidebarCheckboxBtn) {
        sidebarCheckboxBtn.addEventListener('change', toggleSidebar);
    }
